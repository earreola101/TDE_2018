...
if exist "Meridian.txt" (
del Meridian.txt
)
rem exit %Status%
if exist "Meridian.log" (
copy "Meridian.log" "testtracking.log"
rename "Meridian.log" "Meridian.txt"
)
if exist "sn.txt" (
del sn.txt
)
...
set "xprvar="
for /F "skip=5 delims=" %%i in (Meridian.txt) do if not defined xprvar set "xprvar=%%i"   
echo %xprvar%
